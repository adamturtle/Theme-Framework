 ...
 * 
 * @author Markus Thšmmes (merguez@semantifiziert.de)
 * @copyright Copyright 2010, Markus Thšmmes
 * @version 1.3
 * @since 22.08.2010
 * 
 */
...
class AdminPage {
	...
	public function outputHTML() {
		...
		foreach($this->boxes as $box) {
			if($box['type'] != 'title' AND $box['type'] != 'paragraph' AND $box['type'] != 'subtitle' AND $box['type'] != 'success' AND $box['type'] != 'error') {
				if(!$this->table) {
			...
			switch($box['type']) {
				...
				case 'paragraph':
					...
					break;
				case 'success':
					echo '<div id="message" class="updated"><p>'.$box['text'].'</p></div>';
					break;
				case 'error':
					echo '<div id="message" class="error"><p>'.$box['text'].'</p></div>';
					break;
				case 'input':
			...
			if($box['type'] != 'title' AND $box['type'] != 'paragraph' AND $box['type'] != 'subtitle' AND $box['type'] != 'success' AND $box['type'] != 'error' ) echo '</tr>';
		}
